...
function _extract(str, options){
    ...
    if(_.isEmpty(text)){
        ...
        for(var x = 0;x < words.length; x++){
            //  remove question marks, exclamation points, commas, and semi-colons
            var w = words[x].match(/https?:\/\/.*[\r\n]*/g) ? words[x] : words[x].replace(/,|;|!|\?|\(|\)|:|"|^'|'$|“|”|‘|’/g,'');
            //  remove a period if is there is only one and it is the last character in the string
            if(w.indexOf('.') === w.length - 1) w = w.slice(0, -1);
